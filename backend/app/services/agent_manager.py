...
class AgentManager:
    ...
    def get_agent(self, session_id: str, previous_functions: Dict[str, str] = {}, previous_imports: Set[str] = set()) -> Agent:
        """
        Retrieve an existing agent for the session or create a new one.
        """
        if session_id not in self._agents:
            logger.info(f"Creating new Agent for session: {session_id}")
            self._agents[session_id] = Agent(previous_functions=previous_functions, previous_imports=previous_imports)
        else:
